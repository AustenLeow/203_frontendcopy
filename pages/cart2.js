...
export default function Cart2() {
  ...
  const countItemStock = (product) => {
    fetch(`http:// 52.221.188.170:8080/api/v1/items/${product.id}`, {
      method: "GET",
      headers: {
  ...
  async function updateItemQty(item, qty) {
    const response = await fetch(
      `http:// 52.221.188.170:8080/api/v1/cart/update/${item.id}/${qty}`,
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
  ...
  const removeItemHandler = (product) => {
    fetch(`http:// 52.221.188.170:8080/api/v1/cart/delete/${product.id}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
        Authorization: "Bearer " + localStorage.getItem("token"),
